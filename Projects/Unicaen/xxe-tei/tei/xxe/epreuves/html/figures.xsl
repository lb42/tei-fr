...
<xsl:stylesheet
  ...
  <xsl:template match="tei:figure">
	...
	<div>
	  ...
	  <xsl:if test="tei:head">
		...
		<!-- modification EC 17/01/14 pour non répétition de la numérotation des figures-->
  ...
  <xsl:template match="tei:table">
	...
	<xsl:if test="tei:head">
	  <caption>
<!-- modification EC 17/01/14 pour non numérotation des titres de tableaux-->
  ...
  <xsl:template name="showGraphic">
    <xsl:variable name="File">
      <xsl:choose>
        <xsl:when test="@url">
        	<xsl:variable name="graphiclink1" select="substring-after(@url,'../')"/>
        	<xsl:variable name="graphiclink2" select="translate($graphiclink1,'/br/','/')"/>
          <xsl:value-of select="$graphiclink2"/>
