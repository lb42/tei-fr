...
<xsl:stylesheet
  ...
  <xd:doc type="stylesheet">
    ...
    <xd:cvsId>$Id: core.xsl 8154 2010-10-04 18:55:25Z rahtz $</xd:cvsId>
  ...
  <xsl:template match="tei:bibl">
    <xsl:choose>
      ...
      <!-- Ajout PyB 13/01/2011 -->
      <xsl:when test="parent::tei:quote[@type='exemple']">
        <tr>
          <xsl:if test="preceding-sibling::tei:quote/tei:num">
            <td> </td>
          </xsl:if>
          <td class="tabExemple"><span class="bibl"><xsl:apply-templates/></span></td>
        </tr>
      </xsl:when>
      <!-- Fin PyB 13/01/2011 -->
      <xsl:otherwise>
	...
	<span class="bibl" id="{@xml:id}">
          <xsl:apply-templates/>
          <xsl:if test="//tei:ref/@target=$identifier">
	  <a class="link_return"
  ...
  <!-- Début ajout PyB 22/12/2010 -->
  <xsl:template match="tei:list[@type='exempleOrd']">
    <xsl:apply-templates select="tei:item" mode="exemple"/>
  </xsl:template>

  <xsl:template match="tei:item" mode="exemple">
    <table class="exemple">
      <xsl:apply-templates/>
    </table>
  </xsl:template>
  <!-- Fin ajout PyB 22/12/2010 -->
  <xd:doc>
...
<!-- xi:include resolve-->
  <xsl:template match="xi:include">
  	...
  	<xsl:comment><xsl:apply-templates select="$ref"/></xsl:comment>
  	<ab>
  ...
  <xsl:template match="tei:quote">
    <xsl:choose>
      <!-- Début ajout PyB -->
      <xsl:when test="ancestor::tei:list[@type='exempleOrd'] and tei:seg">
        <tr>
          <xsl:apply-templates/>
        </tr>
      </xsl:when>
      <xsl:when test="@type='exemple' and tei:quote/tei:seg">
        <table class="exemple">
          <xsl:apply-templates/>
        </table>
      </xsl:when>
      <xsl:when test="@type='traduction'">
        <xsl:variable name="numSeg">
          <xsl:value-of select="count(preceding-sibling::tei:quote[1]/tei:seg)"/>
        </xsl:variable>
        <tr>
        <xsl:if test="preceding-sibling::tei:quote/tei:num">
          <td> </td>
        </xsl:if>
        <td class="tabExemple" colspan="{$numSeg}">
          <xsl:apply-templates/>
        </td>
      </tr>
      </xsl:when>
      <xsl:when test="parent::tei:quote[@type='exemple'] and tei:seg">
        <tr>
          <xsl:apply-templates/>
        </tr>
      </xsl:when>
      ...
      <!-- Modif PyB -->
      <xsl:when test="parent::tei:p or parent::tei:note or tei:quote">
	  <div class="blockquote">
	     <!-- alignement inverse des quote -->
		  <xsl:if test="contains(@rend,'rtl')">
			<xsl:attribute name="dir">
  ...
  <!-- Début ajout PyB 22/12/2010 -->
  <xsl:template match="tei:quote/tei:num">
    <td class="numExemple">
      <xsl:apply-templates/>
    </td>
  </xsl:template>
  ...
  <xsl:template match="tei:seg">
    <!-- Début ajout PyB -->
    <xsl:choose>
      <xsl:when test="ancestor::tei:quote[@type='exemple']">
        <xsl:if test="parent::tei:quote/preceding-sibling::tei:quote/tei:num and position()=1">
          <td> </td>
        </xsl:if>
        <td class="tabExemple">
          <xsl:apply-templates/>
  ...
  <!-- Début ajout PyB 04/01/2010 -->
  <xsl:template match="tei:index">
    <xsl:choose>
      <xsl:when test="@indexName='marqueurCitation'">
        <xsl:apply-templates/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  <!-- Fin ajout PyB 04/01/2010 -->
  ...
  <xsl:template match="tei:term">
    <xsl:choose>
      ...
      <xsl:when test="@type='identifié' or @type='norm'">
      </xsl:when>
      <!-- Fin ajout PyB 04/01/2010 -->
      <xsl:when test="@rend">
        <xsl:call-template name="rendering"/>
  ...
  <xsl:template name="printNotes">
	...
	<xsl:when test="$footnoteFile='true' and
			ancestor-or-self::tei:TEI/tei:text/descendant::tei:note[@type='standard']">
  ...
  <xsl:template name="makeAnchor">
	...
	<span>
      ...
      <xsl:when test="$name">
	<a name="{$name}">
	  <xsl:comment>anchor</xsl:comment></a>
      </xsl:when>
  ...
  <xsl:template name="rendToClass">
    ...
    <xsl:param name="default">.</xsl:param>
				<!-- récupération de l'attribut langue -->
	...
	<xsl:when test="@rendition">
	  <xsl:call-template name="applyRendition"/>
					...
					<!-- alignement inverse de p -->
    <xsl:choose>
      <xsl:when test="concat($class,$rend)=''"/>
      <xsl:when test="$class=''">
        <xsl:choose>
		  <xsl:when test="contains($rend,'rtl')"> 
            <xsl:attribute name="dir">
              <xsl:text>rtl</xsl:text>
            </xsl:attribute>
          </xsl:when>
