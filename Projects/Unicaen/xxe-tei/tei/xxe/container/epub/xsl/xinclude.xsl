...
<!-- vers l'image de couverture dans un sous-dossier -->
<xsl:template match="/">
		<xsl:result-document href="{concat($dir,'/linksToCover.txt')}" method="text">
...
<xsl:template match="/">
		<xsl:result-document href="{concat($dir,'/linksToCover.txt')}" method="text">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:text>../icono/cover.jpg</xsl:text>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:result-document>
	<xsl:apply-templates/>
...
<xsl:template match="tei:floatingText">
	...
	<xsl:variable name="pathToRef">
		<xsl:value-of select="concat($dir,'/',$ref)"/>
	</xsl:variable>
	<xsl:variable name="floatingSubtype" select="@subtype"/>
	<xsl:variable name="floatInclude">
		<xsl:apply-templates select="document($pathToRef)//tei:text/*"/>
	</xsl:variable>
		<floatingText>
			<xsl:attribute name="type" select="concat('enc_',$floatingSubtype)"/>
			<xsl:copy-of select="$floatInclude"/>
		</floatingText>
</xsl:template> 
...
<xsl:template match="tei:div[@type='liminaires']">
  <div type="liminaires" xml:id="Liminaires">
  <!--xsl:copy-->
	<head>Liminaires</head>
    <xsl:apply-templates/>
    <xsl:if test="following-sibling::tei:div[@type='liminaires']">
    	<xsl:apply-templates select="following-sibling::tei:div[@type='liminaires']/descendant::*"/>
    </xsl:if>
  <!--/xsl:copy-->
  </div>
</xsl:template>

<!-- suppression de l'image de couverture pour éviter le xpath search et parce que la commande l'ajoute au zip -->
<xsl:template match="tei:figure[following-sibling::tei:front]">
</xsl:template>

<xsl:template match="tei:div[@type='liminaires'][preceding-sibling::tei:div[@type='liminaires']]"/>
