...
<!-- this needs to be in the same folder as the profiles/oulipo/html/to.xsl used 
      and it also needs to be explicitly associated with the appropriate driver file in oXygen -->
...
<xsl:template match="tei:divGen[@type='namelist']">
  <table class="sort">
    <thead>
      <tr><th>keycode</th><th>Occurrences</th><th>Noms sans @role</th></tr>
    </thead>
  <!--  <xsl:for-each-group select="//tei:text//tei:persName" group-by="tei:Maybe(@key)">
 -->     <xsl:for-each-group select="//tei:text//tei:persName[not(@role)]" group-by="tei:Norm(.)">
      <xsl:sort select="tei:Norm(.)"/>
      <tr>
  ...
  <table class="sort">
    <thead>
      <tr><th>keycode</th><th>Occurrences</th><th>Noms avec @role</th></tr>
    </thead>
    <!--  <xsl:for-each-group select="//tei:text//tei:persName" group-by="tei:Maybe(@key)">
 -->     <xsl:for-each-group select="//tei:text//tei:persName[@role]" group-by="concat(@role, ' ', tei:Norm(.))">
      <xsl:sort select="tei:Norm(.)"/>
      <tr>
        <td><xsl:value-of select="current-grouping-key()"/></td>
        <td><xsl:value-of select="count(current-group())"/></td>
  ...
  <xsl:function name="tei:Norm" as="xs:string">
    <xsl:param name="val"/> 
  <xsl:variable name="punc">'".,:!?()«»0123456789 []+’</xsl:variable>
  <xsl:variable name="space"><xsl:text> </xsl:text></xsl:variable>
  <xsl:variable name="contract">([A-z]+’)</xsl:variable>
    <xsl:value-of
      select="lower-case(normalize-space(translate(translate($val ,$punc,''),'-',$space)))"/>
  </xsl:function>
